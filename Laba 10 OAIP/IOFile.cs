...
namespace Laba_10_OAIP
{
    public static class IOFile
    {
        ...
        {
            content += "Сравниваем " + Convert.ToString(first) + " и " +
           Convert.ToString(second) + '\n';
            return "Сравниваем " + Convert.ToString(first) + " и " +
           Convert.ToString(second) + '\n';
        }
        public static string InputInfoAboutTransposition(int first, int second)
        {
            content += "Перестановка " + Convert.ToString(first) + " и " +
           Convert.ToString(second) + '\n';
            return "Перестановка " + Convert.ToString(first) + " и " +
           Convert.ToString(second) + '\n';
        }
        ...
        private static void Separator(StreamReader streamReader)
        {
            ...
            try
            {
                while (streamReader.Peek() != -1)
                {
                    ...
                    else if (streamReader.Peek() >= 48 && streamReader.Peek()
                   <= 57)
                    {
                        do
                        {
                        ...
                        string output = new
                       string(input.ToCharArray().Reverse().ToArray());
