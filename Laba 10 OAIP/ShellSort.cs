    ...
    public class ShellSort : IStrategy
    {
        ...
        public int[] Algorithm(int[] mass, bool flag = true)
        {
            if (flag)
            {
               ...
               System.Diagnostics.Stopwatch();
                ...
                for (step = mass.Length / 2; step > 0; step /= 2)
                {
                    ...
                    tmp = mass[i];
                        ...
                        {
                            IOFile.InputInfoAboutComparison(tmp, mass[j -
                           step]);
                            ComparativeAnalysis.Comparison++;
                            if (tmp < mass[j - step])
                            {
                                IOFile.InputInfoAboutTransposition(tmp, mass[j
                               - step]);
                                mass[j] = mass[j - step];
                ...
                myStopwatch.Stop();
                var resultTime = myStopwatch.Elapsed; //затраченное времени
                string elapsedTime =
               ...
               Convert.ToString(ComparativeAnalysis.NumberOfPermutations);
            ...
            int i, j, step;
               ...
               String.Format("{0:00}:{1:00}:{2:00}.{3:000}",
                ...
                resultTime.Seconds,
                resultTime.Milliseconds);
                ComparativeAnalysis.timeSort = resultTime.Seconds * 1000 +
               resultTime.Milliseconds;
